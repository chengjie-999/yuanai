...
# ==================== 审核结果展示函数 ====================
def show_pending_audit_result():
    """检查并展示待显示的审核结果（包括用户反馈）"""
    if PENDING_AUDIT_KEY in st.session_state:
        audit_data = st.session_state[PENDING_AUDIT_KEY]
        ...
        if audit_data.get("source") == "xiao_yuan":
            ...
            st.session_state.messages.append(("assistant", content, display_images))

            del st.session_state[PENDING_AUDIT_KEY]
