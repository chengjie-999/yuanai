...
def get_html(web_driver: WebDriver):
...
def app_go(web_driver: WebDriver):
    """
    自动操作小猿众包
    :return:
    """
    wait = WebDriverWait(web_driver, 10)  # （最长等10秒，每0.5秒轮询一次）每进行一次页面加载时执行一次显式等待
    ...
    for card in cards:
        ...
        # i = 0
        print(title)
        if title.strip() == '单题标答-审核':
            warn = card.find_element(by=By.CSS_SELECTOR, value='.task-card-content .text-warning:nth-child(2)').text
            print(warn)
            ...
            start.click()  # 点击开始任务
            while 1:
                st = go_question(web_driver, wait)  # 单题标答-审核自动化
                if st and st != '1':
                    break
...
def go_question(web_driver, wait: WebDriverWait):
    # 题目 .ol-viewport
    question = wait.until(
    ...
    # 提交任务
    status = input('')
    foot = web_driver.find_element(By.CSS_SELECTOR, '.footer.container_23Xxj')
    if not status:
        button = foot.find_element(By.CSS_SELECTOR, '.ant-space:nth-child(2) .ant-space-item:nth-child(5) .ant-btn')
        button.click()
    elif status == '1':
        button = foot.find_element(By.CSS_SELECTOR, '.ant-space:nth-child(2) .ant-space-item:nth-child(3) .ant-btn')
        button.click()
        # 驳回理由
    else:
        button = foot.find_element(By.CSS_SELECTOR, '.ant-space:nth-child(2) .ant-space-item:nth-child(5) .ant-btn')
