...
from datanalysis.df.core import generate_sample_data
...
def render_page_content(df, total_sales, avg_order, avg_price, growth_rate):
    ...
    if active_page == "数据看板":
        ...
        # 数据下载
        csv_data = df.to_csv(index=False, encoding='utf-8-sig').encode('utf-8-sig')  # 修复1：内容编码
        ...
        st.markdown('<p class="subtitle-text">DatAnalysis 介绍</p>', unsafe_allow_html=True)
...
# --------------------------
# 7. 程序入口（仅当单独运行时执行，被导入时不执行）
# --------------------------
if __name__ == "__main__":
    # 若单独运行这个子脚本，临时加set_page_config（避免报错）
    # 但主脚本调用时，这部分不会执行
    st.set_page_config(
