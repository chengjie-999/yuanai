...
export default function WelcomePage({ images, setImages, quickInput, setQuickInput, sendWithNewSession, sessions, onSelectSession, chatMode, setChatMode, localOnline, claudeOnline }: {
  ...
  return (
    <div className="chat-welcome" style={{
          ...
          >
            <div style={{ display: 'flex', alignItems: 'flex-end', padding: '4px 4px 4px 16px' }}>
              ...
              <button onClick={() => setVoiceOpen(true)} title="语音通话" aria-label="语音通话"
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: '8px', lineHeight: 1, opacity: 0.5, transition: 'opacity 0.15s', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                onMouseEnter={(e) => e.currentTarget.style.opacity = '1'}
                onMouseLeave={(e) => e.currentTarget.style.opacity = '0.5'}
              >
      ...
      {voiceOpen && <VoiceCall onClose={() => setVoiceOpen(false)} />}
