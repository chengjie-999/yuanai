...
function SessionsTab() {
  ...
  const filtered = search.trim()
    ? sessions.filter((s) => s.title?.toLowerCase().includes(search.toLowerCase()) || s.session_id?.includes(search))
    : sessions
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 12, alignItems: 'center' }}>
        <div style={{ flex: 1 }} />
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜索标题或 ID..." style={{ ...inputStyle, width: 220 }} />
      </div>
      {loading ? <Spinner /> : (
        <Card>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead><tr style={{ background: '#f5f5f8' }}>
              {['会话 ID', '标题', '消息数', '创建时间', '更新时间', '操作'].map((h) => <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontSize: 12, color: '#666', fontWeight: 600, borderBottom: '2px solid #e0e0e0' }}>{h}</th>)}
            </tr></thead>
            <tbody>
              {filtered.length === 0 && <tr><td colSpan={6}><Empty msg={search ? '无匹配会话' : '暂无会话'} /></td></tr>}
              {filtered.map((s) => (
                <tr key={s.session_id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                  <td style={{ padding: '10px 14px', fontFamily: 'monospace', fontSize: 11, color: '#888', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.session_id}</td>
                  <td style={{ padding: '10px 14px', fontWeight: 600, color: '#333' }}>{s.title || '未命名'}</td>
