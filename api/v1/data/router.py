...
@router.get("/script-html/{html_id}")
async def serve_script_html(html_id: str):
    """Serve interactive plotly HTML generated by analysis scripts."""
    from utils.data_path import root_path
    SCRIPT_HTML_CACHE_DIR = os.path.join(root_path(), "data", "analysis", "html_cache")
    fpath = os.path.join(SCRIPT_HTML_CACHE_DIR, f"{html_id}.html")
    if not os.path.isfile(fpath):
